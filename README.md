# Visible QWP design data and forward simulations

This repository provides the final structure data, source data underlying
Figures 2--4, Meep forward-simulation scripts for post-optimization
verification, and a three-dimensional rendering script for the visible-light
quarter-wave-plate (QWP) design spaces evaluated in the associated manuscript:
...
└── scripts/
    ...
    └── forward_simulation/
        ├── README.md
        ├── qwp_model.py
        ├── calculate_jones_matrix.py
        └── calculate_field_profiles.py
```
...
The `scripts/forward_simulation/` directory contains the Meep forward
simulations used to verify the final fully free-form QWP after optimization.
The optimization and adjoint-gradient code is not included.

The two executable scripts share the physical model defined in
`scripts/forward_simulation/qwp_model.py`, including:

- the `200 nm × 200 nm` periodic unit cell;
- the `600 nm` design thickness;
- the `500-nm-thick SiO2` substrate with `n = 1.45`;
- the two-pole dispersive TiO2 Lorentz model;
...
The released input structure is:

```text
data/optimized_qwp_binary_structure_3d.npy
```

The expected NumPy shape is `(11, 11, 31)` with axis order `(x, y, z)`,
...
Run a single-wavelength Jones-matrix test at `550 nm` from the repository root:
...
Run the full `500–600 nm` Jones-matrix verification at `1 nm` intervals:
...
Calculate the Figure 6(a) `y = 0` real-field profiles at `500`, `550`, and
`600 nm`:
...
The default field quantity is `real`, corresponding to `Re(Ex)` for
x-polarized incidence and `Re(Ey)` for y-polarized incidence. The previous
amplitude representation remains available with `--field-quantity amplitude`.

For the available polarization-analysis modes, output files, conventions, and
additional execution options, see
[`scripts/forward_simulation/README.md`](scripts/forward_simulation/README.md).
